...
"""
	# cross prodock
	def _crossdot(self, other):
"""

class Matrix2DManager(BaseManager):	pass

class Matrix2DProxy(NamespaceProxy):
	# We need to expose the same __dunder__ methods as NamespaceProxy,
	# in addition to the all methods.
	_exposed_ = ('__getattribute__','__getitem__', '__setitem__', '__str__', '__repr__', '__eq__',
	 '__ne__', '__add__', '__radd__', '__mul__','__rmul__','_print','random','__invert__','__neg__',
	 'write','_issquare','_isunit','_iszero','_iscol0','_isrow0','__setattr__','_trans','__invert__',
	 '_setzero_','_setunit_','_setrandom_')

	def _print(self):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('_print')

	def __add__(self,other):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('__add__')

	def __radd__(self,other):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('__radd__')

	def __setitem__(self,tup,val):
		callmethod = object.__getattribute__(self, '_callmethod')
		self.x , self.y = tup
		return callmethod('__setitem__', (tup, val))

	def __mul__(self,other):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('__mul__', (other,))

	def __rmul__(self,other):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('__rmul__',(other,))

	def __invert__(self):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('__invert__',(other,))		

	def _det(self):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('_det')

	def _trans(self):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('_trans')

	def _setzero_(self):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('_setzero_')

	def _setunit_(self):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('_setunit_')

	def _setrandom_(self, x, y, low, high):
		callmethod = object.__getattribute__(self, '_callmethod')
		return callmethod('_setrandom_',(x,y,low,high,))

## to register manager
Matrix2DManager.register('Matrix2D', Matrix2D, Matrix2DProxy)
...
class testMatrix2D(object):
	...
	def testsharedMatrix(self):
		manager = Matrix2DManager()
		manager.start()
		m1 = manager.Matrix2D(100,100)
		for i in range(100):
			for j in range(100):
				m1[i,j] = random.randrange(0,20)
		m1._print()
		m2 = manager.Matrix2D(100,100)
		for i in range(100):
			for j in range(100):
				m2[i,j] = random.randrange(0,20)
		m1._print()
		m3 = m1 * m2
		m3._print()
		t0 = time.time()
		print m3._det()
		print "det_m3 take" , time.time() - t0 , "seconds, the result is:"
		print m3.__invert__()._det()
		t1 = time.time()
		print "invert_m3 take" , time.time() - t1 , "seconds, the result is:"
		m3.__invert__()._print()
...
"""
	...
	for i in range(100):
		for j in range(100):
			m1[i,j] = random.randrange(0,20)
	#m1._print()
	manager.pool = multiprocessing.Pool(10)
	result = manager.pool.apply_async(unwrap_Matrix2D_det,(m1,))
	manager.pool.close()
	manager.pool.join()
	print result.get()
	#manager.pool.close()
